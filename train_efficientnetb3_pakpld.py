...
RUN_NAME = "runs_efficientnetb3_pakpld"
...
for epoch in range(EPOCHS):
    ...
    if val_acc > best_val_acc:
        ...
        best_epoch = epoch + 1
        no_improve_count = 0
        torch.save(model.state_dict(), MODELS_DIR / "best_efficientnetb3_pakpld.pth")
        print(">> 保存当前最佳模型")
    else:
        no_improve_count += 1
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("EfficientNetB3 Loss Curve (Pak-PLD)")
plt.legend()
...
plt.xlabel("Epoch")
plt.ylabel("Accuracy")
plt.title("EfficientNetB3 Accuracy Curve (Pak-PLD)")
plt.legend()
plt.grid(True, linestyle="--", alpha=0.5)
plt.tight_layout()
...
# 10. 测试集评估
# =========================
best_model_path = MODELS_DIR / "best_efficientnetb3_pakpld.pth"
